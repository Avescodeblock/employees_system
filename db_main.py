import tkinter as tk
from tkinter import messagebox
from tkinter import filedialog
from tkinter import ttk
from PIL import ImageTk,Image
import pymysql
import os
import shutil
import db_config

form = tk.Tk()
form.title("Database Form")

form.geometry("500x280")

tab_parent = ttk.Notebook(form)
tab1 = ttk.Frame(tab_parent)
tab2 = ttk.Frame(tab_parent)

tab_parent.add(tab1, text = "All Records")
tab_parent.add(tab2, text = "Add New Records")

firstLabeltab1 = tk.Label(tab1, text = "First Name:")
familyLabeltab1 = tk.Label(tab1, text = "Family Name:")
jobLabeltab1 = tk.Label(tab1, text = "Job Title:")


firstEntrytab1 = tk.Entry(tab1)
familyEntrytab1 = tk.Entry(tab1)
jobEntrytab1 = tk.Entry(tab1)

imgLabeltab1 = tk.Label(tab1)

buttonForward = tk.Button(tab1, text = "Forward")
buttonBack = tk.Button(tab1, text = "Back")

#ADD WIDGETS TO GRID ON TAB
firstLabeltab1.grid(row = 0, column = 0, padx = 15, pady = 15)
firstEntrytab1.grid(row = 0, column = 1, padx = 15, pady = 15)

familyLabeltab1.grid(row = 1, column = 0, padx = 15, pady = 15)
familyEntrytab1.grid(row = 1, column = 1, padx = 15, pady = 15)


jobLabeltab1.grid(row = 2, column = 0, padx = 15, pady = 15)
jobEntrytab1.grid(row = 2, column = 1, padx = 15, pady = 15)


imgLabeltab1.grid(row = 0, column = 2, rowspan = 3, padx = 15, pady = 15)


tab_parent.pack(expand = 1, fill = "both")



form.mainloop()